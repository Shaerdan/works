...
load('geo.mat');
    dim = 2;
       ...
       th1 = 0:ang_stp:2*pi;
        ...
        for i = 1:n_source
         x_source1a(i) = rr*cos(th1(i));
         x_source2a(i) = rr*sin(th1(i));
        end 
                k=1;
%     plot(x,y); hold on;
       for i = 1:dim:2*n_source-1
         x_source(i) = x_source1a(k);
         x_source(i+1) = x_source2a(k);
         k = k+1;
%     scatter( x_source(i), x_source(i+1)); hold on;
 ...
 %%%% Forward BEM method to solve boundary data phim 
    [ g, pk ] = boundary_pk( r,theta,nx,ny,x,y,x_source,St1,sanity,dim );

    if(reg == 0)
    [ ff,F1,A,P ] = NtD( g,x_source,St1,x,y,xx,yy,...
                                   nx,ny,xm,ym,lx,...
    ...
    elseif (reg == 1)
        Ka = tril(ones(n,n),-1);
        K1 = ones(n,1);
        [ ff,F1,A,P ] = NtD_miu2( g,x_source,St1,x,y,xx,yy,...
                                   nx,ny,xm,ym,lx,...
    ...
    elseif (reg == 2)
        Ka = tril(ones(n,n),-1);
        K1 = ones(n,1);
        [ ff,F1,A,P ] = NtD_miu( g,x_source,St1,x,y,xx,yy,...
                                   nx,ny,xm,ym,lx,...
                                   ly,l,r,theta,sanity,dim,reg_miu,Ka,K1,...
                                   gmres_restart,gmres_tol,gmres_maxit);
...
for i = 1:ee
    ...
    V_jac(i,k+1) =  (St1(j)/2*pi)*xs(j)/ (r_mod1(i,j))^2;
    V_jac(i,k+2) =  (St1(j)/2*pi)*ys(j)/ (r_mod1(i,j))^2;
    k = k + 3;
    end
end
...
for i = 1:mm
    ...
    Gs(i,k+1) =  (St1(j)/2*pi)*xs(j)/ (r_mod2(i,j))^3;
    Gs(i,k+2) =  (St1(j)/2*pi)*ys(j)/ (r_mod2(i,j))^3;
    k = k + 3;
    end
end
[Ua S_a Va] = svd(A);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
       ...
       th1 = 0:ang_stp:2*pi; 
        ...
        for i = 1:n_source
         x_source1b(i) = rr*cos(th1(i));
         x_source2b(i) = rr*sin(th1(i));
        end 
                k=1;
%     plot(x,y); hold on;
       for i = 1:dim:2*n_source-1
...
%     scatter( x_source(i), x_source(i+1)); hold on;
 ...
 %%%% Forward BEM method to solve boundary data phim 
    [ g2, pk2 ] = boundary_pk( r,theta,nx,ny,x,y,x_source22,St2,sanity,dim );

    if(reg == 0)
    [ ff2,F1,A2,P2 ] = NtD( g2,x_source22,St2,x,y,xx,yy,...
                                   nx,ny,xm,ym,lx,...
                                   ly,l,r,theta,sanity,dim,alph_tik);
    ...
    elseif (reg == 1)
        Ka = tril(ones(n,n),-1);
        K1 = ones(n,1);
        [ ff2,F1,A2,P2 ] = NtD_miu2( g2,x_source22,St1,x,y,xx,yy,...
                                   nx,ny,xm,ym,lx,...
                                   ly,l,r,theta,sanity,dim,reg_miu,Ka,K1,...
                                   gmres_restart,gmres_tol,gmres_maxit);
    elseif (reg == 2)
        Ka = tril(ones(n,n),-1);
        K1 = ones(n,1);
        [ ff2,F1,A2,P2 ] = NtD_miu( g2,x_source22,St1,x,y,xx,yy,...
                                   nx,ny,xm,ym,lx,...
                                   ly,l,r,theta,sanity,dim,reg_miu,Ka,K1,...
                                   gmres_restart,gmres_tol,gmres_maxit);
...
for i = 1:ee
    ...
    V_jac2(i,k+1) =  (St2(j)/2*pi)*xs(j)/ (r_mod3(i,j))^2;
    V_jac2(i,k+2) =  (St2(j)/2*pi)*ys(j)/ (r_mod3(i,j))^2;
    k = k + 3;
    end
end
...
for i = 1:mm
    ...
    Gs2(i,k+1) =  (St2(j)/2*pi)*xs(j)/ (r_mod4(i,j))^3;
    Gs2(i,k+2) =  (St2(j)/2*pi)*ys(j)/ (r_mod4(i,j))^3;
    k = k + 3;
    end
end
[Ua2 S_a2 Va2] = svd(A2);
