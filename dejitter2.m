...
lb = -1*ones(1,n_s+1);
ub = -lb;
    x_soln = simulannealbnd(@(x)shiftingpixel2(x),x0,lb,ub,opts);
elseif (method ==4)
opts = optimoptions('ga','ConstraintTolerance',1e-6,'PlotFcn', @gaplotbestf);
x_soln = ga(@(x)shiftingpixel2(x),n_s+1,[],[],[],[],[],[],[],opts);
